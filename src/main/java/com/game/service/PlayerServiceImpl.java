...
public class PlayerServiceImpl implements PlayerService {
    ...
    @Override
    public Player save(Player findPlayer, Player updatePlayer) {
        ...
        if (updatePlayer.getExperience() != null && updatePlayer.getExperience() < 0) return null;
        if (updatePlayer.getBirthday() != null && updatePlayer.getBirthday().getTime() < 0 &&
                !(updatePlayer.getBirthday().after(new GregorianCalendar(2000, Calendar.JANUARY, 1).getTime()) &&
                        updatePlayer.getBirthday().before(new GregorianCalendar(3000, Calendar.DECEMBER, 31).getTime())))
            return null;


        if (updatePlayer.getName() != null && (updatePlayer.getName().length() < 12 || !"".equals(updatePlayer.getName())))
